...
public class JmsWildcardsProducerB {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            //destination = session.createQueue("xiangxue.vip.program.redis.cache");
            destination = session.createTopic("xiangxue.vip.program.redis.cache");
            messageProducer = session.createProducer(destination);
            for(int i=0;i<SENDNUM;i++){
                String msg = "program.redis:"+i+" "+System.currentTimeMillis();
